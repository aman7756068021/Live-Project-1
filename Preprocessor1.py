...
def fetch_time_features(df):
    ...
    # Financial year
    df["Financial_Year"] = df.apply(lambda x: f"{
                                    x["Year"]} - {x["Year"]+1}" if x["Month"] >= 4 else f"{x["Year"]-1} - {x["Year"]}", axis=1)
    return df
...
def fetch_top_revenue_retailers(df):
    retailers_revenue = df[["Retailer", "Amount"]].groupby(
        "Retailer").sum().reset_index().sort_values(by="Amount", ascending=False)
    ...
    retailers = pd.DataFrame(
        data={"percentage revenue": percentages, "retailer_count": retailer_count})
    return retailers
...
def fetch_top_revenue_companies(df):
    company_revenue = df[["Company", "Amount"]].groupby(
        "Company").sum().reset_index().sort_values(by="Amount", ascending=False)
    ...
    companies = pd.DataFrame(
        data={"percentage revenue": percentages, "company_count": company_count})
    return companies
